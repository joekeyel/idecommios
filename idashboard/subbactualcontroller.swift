...
class subbactualcontroller: UIViewController ,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate{
   
    

    @IBOutlet weak var subbactualtable: UITableView!
    @IBOutlet weak var searchbaractual: UISearchBar!
    var listsubbobject:[subbactualobject] = []
    
    var filteredsubbactual:[subbactualobject]? = []
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "subbactualcell", for: indexPath) as! subbactualcell
        
        if isSearching{
            
            cell.newcab.text = self.filteredsubbactual![indexPath.item].newcabinet
            cell.region.text = self.filteredsubbactual![indexPath.item].region
            cell.oldcab.text = self.filteredsubbactual![indexPath.item].oldcabinet
            ...
            cell.newcab.text = self.listsubbobject[indexPath.item].newcabinet
            cell.region.text = self.listsubbobject[indexPath.item].region
            cell.oldcab.text = self.listsubbobject[indexPath.item].oldcabinet
    ...
    func fetchsummary(){
        ...
        let task = URLSession.shared.dataTask(with: urlrequest){(data,response,error)  in
            ...
            if let data = data {
                ...
                self.listsubbobject = [subbactualobject]()
                ...
                do{
                    ...
                    if  let summaryfromjson  = json["Subb"] as? [[String:AnyObject]]{
                        ...
                        for summaryfromjson in summaryfromjson {
                            let projectobj = subbactualobject()
                            if let newcabinet = summaryfromjson["newsubb"] as? String,
                                let oldcabinet = summaryfromjson["oldsubb"] as? String,
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.subbactualtable.reloadData()
